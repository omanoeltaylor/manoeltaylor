          ...
          <div key={project.id} className={`group relative ${idx % 2 !== 0 ? 'md:mt-32' : ''}`}>
            <div className="relative mb-8 border-8 border-white brutalist-shadow overflow-hidden rotate-[-1deg] group-hover:rotate-0 transition-transform duration-500">
              <img 
                ...
                alt={project.title} 
                className="w-full aspect-video object-cover grayscale group-hover:grayscale-0 transition-all duration-700 group-hover:scale-110"
                referrerPolicy="no-referrer"
              />
              <div className="absolute top-4 right-4 bg-accent-red text-white px-6 py-2 font-black text-2xl brutalist-shadow">
                {project.year}
              </div>
            </div>
            <div className="relative z-10">
              ...
              <p className="text-xl text-white font-bold leading-tight uppercase max-w-xl">
                {project.description}
              </p>
            </div>
            {/* Background Number */}
            <div className="absolute -bottom-10 -right-10 text-[15vw] opacity-5 font-black pointer-events-none select-none text-stroke">
              0{idx + 1}
            </div>
          </div>
        ))}
