...
import { getLectures } from '../lib/content';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-20">
        <div className="lg:col-span-7">
          <div className="relative mb-20">
            ...
            <div className="absolute -top-10 -left-10 text-[20vw] opacity-5 font-black pointer-events-none select-none text-stroke">VOZ</div>
          </div>
          ...
          <div className="space-y-16">
            {LECTURES.map((lecture, idx) => (
              <div key={lecture.id} className="group relative border-l-8 border-accent-yellow pl-8 py-4 bg-secondary-bg p-8 brutalist-shadow-red hover:translate-x-4 transition-transform">
                <div className="absolute -top-4 -right-4 bg-white text-black w-12 h-12 flex items-center justify-center font-black text-2xl border-4 border-black">{idx + 1}</div>
                <h2 className="text-4xl md:text-5xl mb-8 leading-none uppercase font-black">{lecture.title}</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                  <div className="flex items-center gap-3 text-white font-black uppercase"><Users size={24} className="text-accent-red" /><span>{lecture.audience}</span></div>
                  <div className="flex items-center gap-3 text-white font-black uppercase"><Clock size={24} className="text-accent-red" /><span>{lecture.duration}</span></div>
                  <div className="flex items-center gap-3 text-white font-black uppercase"><MapPin size={24} className="text-accent-red" /><span>{lecture.format}</span></div>
                </div>
                <p className="text-xl text-gray-400 font-bold uppercase leading-tight">{lecture.description}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-5">
          <div className="bg-white text-black p-10 md:p-16 brutalist-shadow-accent h-fit sticky top-32 rotate-1">
            ...
            <form className="space-y-8">
              ...
              <button className="w-full py-8 bg-black text-white font-black text-3xl hover:bg-accent-red transition-all brutalist-shadow active:translate-y-2 active:shadow-none">ENVIAR PEDIDO</button>
            </form>
          </div>
        </div>
      </div>
