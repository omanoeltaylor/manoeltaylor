...
import { ARTWORKS, PRODUCTS, ARTICLES } from '../constants';

const Home = () => {
  return (
    <div className="pt-0">
      {/* Hero Section */}
      <section className="min-h-[80vh] flex flex-col justify-center items-center relative px-6 overflow-hidden">
        <motion.h1 
      ...
      <section className="bg-white text-black py-20 md:py-40 relative">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20">
          <div>
            <h2 className="font-display font-black uppercase leading-[0.8] tracking-tighter text-6xl sm:text-8xl md:text-[10rem] mb-8 md:mb-12">QUEM?</h2>
          ...
          <div className="flex flex-col justify-center">
            <p className="text-xl font-bold leading-tight mb-8">
              Manoel Taylor é um contador de histórias visual obcecado pelas bordas cruas da vida urbana e pelo potencial inexplorado da ficção de gênero como ferramenta de resistência.
            </p>
      ...
      <section className="py-12 md:py-20">
        <h2 className="text-4xl sm:text-6xl md:text-8xl mb-8 md:mb-12">CONTEÚDO RECENTE</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
          {ARTICLES.map((article) => (
            <Link key={article.id} to={`/content/${article.id}`} className="group">
              ...
              <h3 className="text-2xl md:text-4xl group-hover:text-accent-yellow transition-colors">{article.title}</h3>
            </Link>
          ))}
        </div>
      </section>
